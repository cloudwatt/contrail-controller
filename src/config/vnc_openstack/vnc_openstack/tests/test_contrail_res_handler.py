...
class TestResourceCreateHandler(TestContrailBase):
    ...
    def test__resource_create_already_exists(self):
        def foo_create(obj):
            if obj.uuid == 'foo-uuid':
                raise vnc_exc.RefsExistError()
            return 'bar-uuid'

        self.vnc_lib.foo_create.side_effect = foo_create
        obj = mock.Mock()
        obj.name = 'foo-name'
        obj.uuid = 'foo-uuid'
        obj.fq_name = ['default-domain', 'project', 'foo-name']
        uuid = self.create_handler._resource_create(obj)
        self.assertEqual('bar-uuid', uuid)
...
class TestVMachineHandler(TestContrailBase):
    ...
    def _test_ensure_vm_instance_helper(self, instance_id, fake_instance_obj,
        ...
        if get_ret_val:
            self.vm_handler._resource_get.return_value = get_ret_val

        if get_side_effect:
            self.vm_handler._resource_get.side_effect = get_side_effect
    ...
    def test_ensure_vm_instance(self):
        self._test_ensure_vm_instance_helper('foo-id', 'fake-inst-obj',
                                             get_side_effect=None,
                                             get_ret_val='fake-inst-obj')
        self.vm_handler._resource_get.assert_called_once_with(id='foo-id')

    def test_ensure_vm_instance_not_present(self):
        self.vm_handler._resource_create = mock.Mock()
        self._test_ensure_vm_instance_helper(
            'foo-id', 'fake-obj',
            get_side_effect=vnc_exc.NoIdError(mock.ANY),
            get_ret_val=None)
        self.vm_handler._resource_create.assert_called_once_with('fake-obj')

    def test_ensure_vm_instance_uuid_not_present(self):
        self.vm_handler._resource_create = mock.Mock()
        instance_obj = mock.Mock()
        instance_id = str(uuid.uuid4())
        self._test_ensure_vm_instance_helper(
            instance_id, instance_obj,
            get_side_effect=vnc_exc.NoIdError(mock.ANY),
            get_ret_val=None)
        self.vm_handler._resource_create.assert_called_once_with(instance_obj)
        self.assertEqual(instance_id, instance_obj.uuid)
